...
- **Threshold injury GM setup**: The `thresholdInjuries` world setting is disabled by default. When enabled, trusted normal attack-card damage can qualify through an inclusive upper-half original damage roll, applied damage greater than half the target's maximum HP, or a dedicated natural-20 critical damage action. Natural-20 critical damage applies double damage and creates the threshold injury outcome automatically when the normal trust, permission, and below-zero wound-preemption gates allow it. Other qualifying hits roll `1d10 >= 9 + injuryResistance - edge`; Edge comes from attack margin against current AAC, with a natural 20 counting as Edge 3 for context. `injuryResistance` is manually assigned on character and monster sheets; use 0 for unprotected/light, 1 for medium/modest natural armor, 2 for heavy/strong natural armor, and 3 for rare exceptional protection. Weapon damage profile also affects triggered injury severity.
